...
function fieldAccess(ev, ast, left, right) {
  ...
  if (leftType !== ev.dynType) return leftType.field(left, right, ast, ev)
      ...
      switch (type) {
        ...
        default:
          ...
          if (type.kind === 'message' && valueType.kind === 'map') {
            const converted = ev.objectTypes.get(type.name).convert?.(value)
            if (converted) return ctx.setConverted(ast.args, converted)
          }
      }
