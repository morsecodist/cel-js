...
export class TypeDeclaration {
  ...
  #getMessageField(obj, key, ast, ev) {
    ...
    const type = message.fields ? message.fields[key] : celTypes.dyn
    if (!type) return undefined

    const value = message.withDefaultConvert ? obj.get(key) : obj[key]
...
export class Registry {
  ...
  registerVariable(name, type, opts) {
    ...
    if (
      ...
      if (type.schema) type = this.#registerSchemaAsType(`$${name}`, type.schema).type
      else type = type.type
    } else if (typeof name === 'object') {
      if (name.schema) type = this.#registerSchemaAsType(`$${name.name}`, name.schema).type
      else type = name.type
  ...
  #registerSchemaAsType(name, schema) {
    ...
    for (const key of objKeys(schema)) {
      ...
      if (typeof fieldDef === 'string') {
        ...
        fields[key] = this.#registerSchemaAsType(`${name}.${key}`, fieldDef).name
      } else {
        throw new Error(`Invalid field definition for '${name}.${key}'`)
      }
    }
    return this.registerType(name, {fields})
  }
  ...
  registerType(name, _d) {
    ...
    const decl = {
      ...
      fields: this.#normalizeFields(name, typeof _d === 'function' ? undefined : _d?.fields)
    }
  ...
  #createDefaultConvert(name, fields) {
    ...
    const names = keys.map((k) => JSON.stringify(k))
    const lines = ['    if (v instanceof Map) {']
    for (const k of names) lines.push(`      this[${k}] = v.get(${k})`)
    lines.push('    } else {')
    for (const k of names) lines.push(`      this[${k}] = v[${k}]`)
    lines.push('    }')
    ...
    const Ctor = new Function(
      'verifiedProto',
      [
        `return class Message_${name.replace(/[^a-zA-Z0-9_$]/g, '_')} {`,
        '  #verified = Object.create(verifiedProto)',
        `  constructor(v) {`,
        lines.join('\n'),
        `  }`,
        `  get (field) {`,
        `    switch (this.#verified[field]) {`,
        `      case undefined: return`,
        `      case true: return this[field]`,
        `      default: {`,
        `         this[field] = this.#verified[field].convert(this[field])`,
        `         this.#verified[field] = true`,
        `         return this[field]`,
        `      }`,
        `    }`,
        `  }`,
...
export class RootContext {
  ...
  #getValueFromObject(key) {
    const v = this.#convertCache?.get(key) || this.#context[key]
    if (v !== undefined) return v
    return this.#constants.get(key)
  }

  #getValueFromMap(key) {
    const v = this.#convertCache?.get(key) || this.#context.get(key)
    if (v !== undefined) return v
    return this.#constants.get(key)
  }
  ...
  setConverted(name, value) {
    const cache = (this.#convertCache ??= new Map())
...
class OverlayContext {
  ...
  setConverted(name, value) {
    if (this.iterVar === name) this.iterValue = value
    else this.#parent.setConverted(name, value)
    return value
  }
